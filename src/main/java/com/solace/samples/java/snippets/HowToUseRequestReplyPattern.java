...
public class HowToUseRequestReplyPattern {
   ...
   * @param requestsDestination where to send a request (it is same for requests and responses)
   */
  private static void publishRequestAndProcessResponseMessageAsync$(MessagingService service,
      Topic requestsDestination) {
    // 30 seconds
    ...
    // 'requestsDestination' topic and optionally provided user context 'someUserContextAtPublishTime'
    requester.publish(pingMessage, replyMessageHandler, someUserContextAtPublishTime,
        requestsDestination, replyTimeout);
  ...
  private static void publishRequestAndProcessResponseMessageBlocking$(MessagingService service,
    ...
    // 2) create a request message
    final OutboundMessage pingMessage = service.messageBuilder().build("Ping");
    // 3) finally publish a request, uses  'requestsDestination' topic, method call is blocking
    try {
      requester.publishAwaitResponse(pingMessage, requestsDestination, replyTimeout);
